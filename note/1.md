...
```js
Cannot use [chunkhash] or [contenthash] for chunk in '[name]_[chunkhash:8].js'
```
* 如果 build 的时候，如果使用了 `chunkhash` 的 `plugins` 上不能有 `new webpack.HotModuleReplacementPlugin()`
